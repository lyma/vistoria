...
original_author="lyma"
original_name="vistoria"
original_urlname="vistoria"
original_description="Awesome vistoria created by lyma" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/vistoria "${name}"
...
then
    ...
    echo "then run 'vistoria' or 'python -m vistoria'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'vistoria' or 'python -m vistoria' or 'poetry run vistoria'"
fi
