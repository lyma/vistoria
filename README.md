...
- 🎯 Entry points to execute your program using `python -m <vistoria>` or `$ vistoria` with basic CLI argument parsing.
...
---
# vistoria Flask Application

Awesome vistoria created by lyma
...
```bash
git clone https://github.com/lyma/vistoria vistoria
cd vistoria
make install
```
...
```bash
vistoria create-db   # run once
vistoria populate-db  # run once (optional)
vistoria add-user -u admin -p 1234  # ads a user
vistoria run
```
